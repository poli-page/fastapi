...
DEMO_VERSION = "1.0.0"
DEMO_DATA: dict[str, Any] = {"name": "FastAPI demo"}
...
<div class="page">
  ...
  <section>
    <div class="head">
      ...
      <p class="desc">
        ...
        binary PDF (<code>render.pdf</code>), streamed PDF (<code>render.pdf_stream</code>),
  ...
  <section>
    <div class="head">
      ...
      <p class="desc">
        <code>poli_page.fs.render_to_file</code> streams the PDF straight to disk under
  ...
  <section>
    <div class="head">
      ...
      <p class="desc">
        ...
        <code>INVALID_VERSION_FORMAT</code>. The opt-in
        <code>poli_page_exception_handler</code> propagates the API code, message,
        and request-id as JSON.
      </p>
    </div>
    <div class="actions">
  ...
  <section>
    ...
    <div class="cli">
      <button class="copy" data-copy="uv run uvicorn main:app --reload --port 8000">Copy</button>
      <span class="prompt">$</span>uv run uvicorn main:app --reload --port 8000
    </div>
    <div class="cli">
      <button class="copy" data-copy="curl -sSL http://localhost:8000/render/pdf -o /tmp/welcome.pdf && open /tmp/welcome.pdf">Copy</button>
      <span class="prompt">$</span>curl -sSL http://localhost:8000/render/pdf -o /tmp/welcome.pdf &amp;&amp; open /tmp/welcome.pdf
    </div>
  </section>
...
(() => {
  ...
  const setDocId = id => {
    ...
    if (id) {
      ...
      text.innerHTML = `document_id: ${id} <button class="copy" data-copy="${id}">copy</button>`;
    } else {
  ...
  const actions = {
    ...
    'doc-create': async target => {
      ...
      if (r.ok) { try { setDocId(JSON.parse(body).document_id); } catch {} }
    },
...
def render_pdf(client: Client) -> PdfResponse:
...
def render_stream(client: Client) -> PdfStreamResponse:
    def gen() -> Any:
...
def render_to_file(client: Client) -> dict[str, str]:
...
def render_preview(client: Client) -> PreviewResponse:
...
def document_create(client: Client) -> dict[str, Any]:
...
def document_get(document_id: str, client: Client) -> DocumentRedirectResponse:
...
def document_thumbnails(document_id: str, client: Client) -> dict[str, list[dict[str, Any]]]:
...
def document_preview(document_id: str, client: Client) -> PreviewResponse:
...
def document_delete(document_id: str, client: Client) -> Response:
...
def error_bad_version(client: Client) -> JSONResponse:
    client.render.pdf(
        {
            "project": DEMO_PROJECT,
            "template": DEMO_TEMPLATE,
